...
const getSale = async (page, startDate, endDate) => {
    const result = await apiServices.getSale(page, startDate, endDate);
    renderResult(result.data);
    pagination(result.page || 1, result.total_page || 1, window.location.origin, "/hist-jual.html", `?start-date=${$('#startDate').val()}`, `&end-date=${$('#endDate').val()}&`);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    $('#refresh-btn').on('click', () => {
        window.location.replace(`${window.location.origin}/hist-jual.html?page=1&start-date=${$('#startDate').val()}&end-date=${$('#endDate').val()}`);
    })

    getSale(paramPage, dateFormat(startDateParam, 'yyyy-mm-dd'), dateFormat(endDateParam, 'yyyy-mm-dd'));
});
...
function renderResult(result) {
    ...
    result.forEach(element => {
        inner = inner + `
            <tr class="tr-shadow">
                <td>${element.name}</td>
                <td>${element.qyt} ${element.unit}</td>
                <td>Rp${element.price.replace('$', '').replace('.00', ',-').replace(',', '.')}</td>
                <td>Rp${element.discount.replace('$', '').replace('.00', ',-')}</td>
                <td>Rp${element.price_total.replace('$', '').replace('.00', ',-').replace(',', '.')}</td>
